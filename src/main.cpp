...
int main(){
    ...
    */
    ...
    std::cout << "WELCOME TO CS225 FINAL PROJECT BY teamSOCK !!!" << std::endl;
    ...
    std::cout << "KRUSKALS'S ALGORITHM TO FIND THE SHORTEST PATH BETWEEN TWO SUBREDDITS" << std::endl;
    ...
    bool runner = true;
    do{
        std::cout << "TYPE 1 FOR MOST VISITED" << std::endl;
        ...
        do{
            ...
            if(it == workaround.end()){
                std::cerr << "Enter a valid choice" << std::endl;
            }
        }while(it == workaround.end());
        if(algochoice == 1){
            std::string mostvisted = g.MostVisited();
            std::cout << "THE MOST VISITED SUBREDDIT IS: " << mostvisted <<std::endl; 
        }else if(algochoice == 2){
            std::string origin = "godeater";
            std::cout << "ENTER A START SUBREDDIT" << std::endl;
            ...
            do{
                ...
                if(mit == graph.end()){
                    std::cerr << "Enter a valid subreddit / Subreddit not present in dataset" << std::endl;
                }
            }while(mit == graph.end());
            std::string dest = "godeater";
            std::cout << "ENTER A END SUBREDDIT" << std::endl;
            ...
            std::cout << "THE SHORTEST PATH BETWEEN " << origin << " AND " << dest << " IS: " << shortpath << std::endl;
        ...
        std::string yesorno;
        std::cout << "WOULD YOU LIKE TO CONTINUE (y or n)" << std::endl;
